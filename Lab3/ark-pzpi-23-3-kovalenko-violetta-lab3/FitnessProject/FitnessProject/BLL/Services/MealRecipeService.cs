...
public class MealRecipeService : IMealRecipeService
{
    ...
    public async Task<MealRecipeDetailsDto?> GetMealRecipeDetailsByIdAsync(int mealId, int recipeId)
    {
        var entity = await _repository.GetMealRecipeDetailsByIdAsync(mealId, recipeId);
        return entity == null ? null : _mapper.Map<MealRecipeDetailsDto>(entity);
    }
